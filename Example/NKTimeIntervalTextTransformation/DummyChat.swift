...
class DummyChat: UITableViewController{
  ...
  override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
    ...
    if cell.lblTime != nil { cell.lblTime.text = Date.timeIntervalFromNow(to: dates[indexPath.row], with: timeIntervalConfiguration) }
    
    if cell.lblMessage != nil { cell.lblMessage.text = "This message was written on \(dateFormatter.string(from: dates[indexPath.row])). Label in the top right corner says how much time has passed since now." }
